...
input=/app/lbs/traj/caoyanbin/ssid/ssid_poi_pattern.txt/
#INPUT=/app/lbs/traj/zhanghuaizhi/conn_mark/20170606.apxy_joined/part-00000.gz
OUTPUT=/app/lbs/traj/caoyanbin/ssid/ssid_pattern_new.txt/

$HADOOP_HOME/bin/hadoop dfs -D $UGI -rmr $OUTPUT
$HADOOP_HOME/bin/hadoop streaming\
	-input $input\
	-output $OUTPUT \
	-mapper "$RUNPY mapper_poi.py"\
	-reducer "$RUNPY -v reducer_poi2.py" \
	$JOBUGI \
    -jobconf mapred.job.name="traj_pattern-${date}_ssid_caoyanbin" \
	-jobconf mapred.map.tasks=1500\
	-jobconf mapred.reduce.tasks=1200\
